...
import {
  buildInitStateRecursively,
  getFormElementsRecursively
} from "../../services/form";
import "./form.css";
...
class Form extends Component {
  constructor(props) {
    super(props);
    this.state = { inputs: [], loading: false, form: undefined };
  }
  ...
  getTaskContract = (task_Id) => {
    ...
    get(`/bonita/API/bpm/userTask/${task_Id}/contract`, undefined, true)
      .then((res) => {
        const { inputs } = res.data;
        const initFormState = buildInitStateRecursively(inputs);
        this.setState({
          inputs,
          form: { requestInputs: { employeeName: "" } },
          loading: false
        });
      })
      .catch((e) => {
